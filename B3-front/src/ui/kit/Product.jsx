...
export const Product = ({product, amount, ownerStruct, isOrderProduct=false, isInPossession=false, inStock=false}) => {
    ...
    const cardBody = (
        <Card.Body>
            ...
            {!!ownerStruct && <Card.Text>Количество: {ownerStruct.products[product.id]}</Card.Text>}
    ...
    const handleConfirmProduct = async (ev) => {
        ...
        const minOrderCount = +ev.target[1].value;
        const maxOrderCount = +ev.target[2].value;
        ...
        ((user.role === "CLIENT" && (users[executor].role === "DISTRIBUTOR" || users[executor].role === "OPERATOR"))
            ...
            <>
                {!product.confirmed ? (
                    ...
                    <>
                        {users[executor].role !== "SUPPLIER" && <Card.Text>В наличии: {users[executor].products[product.id] ?? 0} шт.</Card.Text>}
                        {!isOrderProduct && !isInPossession && (
                            <Card.Footer>
